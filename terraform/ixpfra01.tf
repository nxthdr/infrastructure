...
resource "docker_network" "ixpfra01_backend" {
  name     = "backend"
...
# Alloy
resource "docker_image" "ixpfra01_alloy" {
  name     = "grafana/alloy:v1.11.2"
  provider = docker.ixpfra01
}

resource "docker_container" "ixpfra01_alloy" {
  image    = docker_image.ixpfra01_alloy.image_id
  name     = "alloy"
  provider = docker.ixpfra01
  command = [
...
# Node Exporter
resource "docker_image" "ixpfra01_node_exporter" {
  name     = "prom/node-exporter:v1.10.0"
  provider = docker.ixpfra01
}

resource "docker_container" "ixpfra01_node_exporter" {
  image    = docker_image.ixpfra01_node_exporter.image_id
  name     = "node_exporter"
  provider = docker.ixpfra01
  command = [
  ...
  user         = "1000:1000"
  pid_mode     = "host"
  hostname     = "ixpfra01" # Also fix this hostname if it should be dynamic
  network_mode = "bridge"
  networks_advanced {
    name = docker_network.ixpfra01_backend.name
  }
...
# Cadvisor
resource "docker_image" "ixpfra01_cadvisor" {
  name     = "gcr.io/cadvisor/cadvisor:v0.52.1"
  provider = docker.ixpfra01
}

resource "docker_container" "ixpfra01_cadvisor" {
  image    = docker_image.ixpfra01_cadvisor.image_id
  name     = "cadvisor"
  provider = docker.ixpfra01
  restart = "unless-stopped"
  log_driver = "json-file"
...
# Tailscale
resource "docker_image" "ixpfra01_tailscale" {
  name     = "tailscale/tailscale:latest"
  provider = docker.ixpfra01
}

resource "docker_container" "ixpfra01_tailscale" {
  image    = docker_image.ixpfra01_tailscale.image_id
  name     = "tailscale"
  provider = docker.ixpfra01
  restart = "unless-stopped"
  log_driver = "json-file"
