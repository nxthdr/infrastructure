...
resource "docker_network" "ixpcdg02_backend" {
  name     = "backend"
...
resource "docker_container" "ixpcdg02_alloy" {
  image    = docker_image.ixpcdg02_alloy.image_id
  name     = "alloy"
  provider = docker.ixpcdg02
  command = [
  ...
  network_mode = "bridge"
  networks_advanced {
    name = docker_network.ixpcdg02_backend.name
  }
  ports {
    internal = 514
    external = 514
...
# Node Exporter
resource "docker_image" "ixpcdg02_node_exporter" {
  name     = "prom/node-exporter:v1.10.2"
  provider = docker.ixpcdg02
}

resource "docker_container" "ixpcdg02_node_exporter" {
  image    = docker_image.ixpcdg02_node_exporter.image_id
  name     = "node_exporter"
  provider = docker.ixpcdg02
  command = [
  ...
  user         = "1000:1000"
  pid_mode     = "host"
  hostname     = "ixpcdg02" # Also fix this hostname if it should be dynamic
  network_mode = "bridge"
  networks_advanced {
    name = docker_network.ixpcdg02_backend.name
  }
...
# Cadvisor
resource "docker_image" "ixpcdg02_cadvisor" {
  name     = "gcr.io/cadvisor/cadvisor:v0.55.1"
  provider = docker.ixpcdg02
}

resource "docker_container" "ixpcdg02_cadvisor" {
  image    = docker_image.ixpcdg02_cadvisor.image_id
  name     = "cadvisor"
  provider = docker.ixpcdg02
  restart = "unless-stopped"
  log_driver = "json-file"
...
# Tailscale
resource "docker_image" "ixpcdg02_tailscale" {
  name     = "tailscale/tailscale:latest"
  provider = docker.ixpcdg02
}

resource "docker_container" "ixpcdg02_tailscale" {
  image    = docker_image.ixpcdg02_tailscale.image_id
  name     = "tailscale"
  provider = docker.ixpcdg02
  restart = "unless-stopped"
  log_driver = "json-file"
