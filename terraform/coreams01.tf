...
resource "docker_image" "clickhouse" {
  name = "docker.io/clickhouse/clickhouse-server:26.4.2"
  provider = docker.coreams01
}
...
resource "docker_image" "headplane" {
  name = "ghcr.io/tale/headplane:0.6.3"
  provider = docker.coreams01
}
...
resource "docker_image" "synapse" {
  name = "matrixdotorg/synapse:v1.152.1"
  provider = docker.coreams01
}
...
resource "docker_image" "mas" {
  name = "ghcr.io/element-hq/matrix-authentication-service:1.17.0"
  provider = docker.coreams01
}
...
resource "docker_image" "cinny" {
  name = "ghcr.io/cinnyapp/cinny:v4.12.1"
  provider = docker.coreams01
}
