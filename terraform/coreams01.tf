...
resource "docker_image" "clickhouse" {
  name = "docker.io/clickhouse/clickhouse-server:26.3.9"
  provider = docker.coreams01
}
...
resource "docker_image" "redpanda" {
  name = "redpandadata/redpanda:v25.3.14"
  provider = docker.coreams01
}
...
resource "docker_image" "alertmanager" {
  name = "prom/alertmanager:v0.32.0"
  provider = docker.coreams01
}
...
resource "docker_image" "headplane" {
  name = "ghcr.io/tale/headplane:0.6.2"
  provider = docker.coreams01
}
...
resource "docker_image" "synapse" {
  name = "matrixdotorg/synapse:v1.152.0"
  provider = docker.coreams01
}
...
resource "docker_image" "mas" {
  name = "ghcr.io/element-hq/matrix-authentication-service:1.16.0"
  provider = docker.coreams01
}
