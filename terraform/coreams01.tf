...
resource "docker_image" "clickhouse" {
  name = "docker.io/clickhouse/clickhouse-server:26.6.2"
  provider = docker.coreams01
}
...
resource "docker_image" "redpanda" {
  name = "redpandadata/redpanda:v26.1.13"
  provider = docker.coreams01
}
...
resource "docker_image" "alertmanager" {
  name = "prom/alertmanager:v0.33.1"
  provider = docker.coreams01
}
...
resource "docker_image" "node_exporter" {
  name = "prom/node-exporter:v1.12.1"
  provider = docker.coreams01
}
...
resource "docker_image" "headplane" {
  name = "ghcr.io/tale/headplane:0.7.0"
  provider = docker.coreams01
}
...
resource "docker_image" "synapse" {
  name = "matrixdotorg/synapse:v1.157.1"
  provider = docker.coreams01
}
...
resource "docker_image" "mas" {
  name = "ghcr.io/element-hq/matrix-authentication-service:1.21.0"
  provider = docker.coreams01
}
...
resource "docker_image" "cinny" {
  name = "ghcr.io/cinnyapp/cinny:v4.12.3"
  provider = docker.coreams01
}
