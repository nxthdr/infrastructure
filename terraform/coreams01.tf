...
resource "docker_image" "clickhouse" {
  name = "docker.io/clickhouse/clickhouse-server:25.8.2"
  provider = docker.coreams01
}
...
resource "docker_image" "chproxy" {
  name = "contentsquareplatform/chproxy:v1.29.0"
  provider = docker.coreams01
}
...
resource "docker_image" "redpanda" {
  name = "redpandadata/redpanda:v25.2.2"
  provider = docker.coreams01
}
