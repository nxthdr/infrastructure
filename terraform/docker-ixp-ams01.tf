...
resource "docker_container" "ixp_ams01_node_exporter" {
  image = docker_image.ixp_ams01_node_exporter.image_id
  name  = "node_exporter"
  provider = docker.ixp_ams01
  command = [
...
# Cadvisor
resource "docker_image" "ixp_ams01_cadvisor" {
  name = "gcr.io/cadvisor/cadvisor:v0.52.1"
  provider = docker.ixp_ams01
}

resource "docker_container" "ixp_ams01_cadvisor" {
  image = docker_image.ixp_ams01_cadvisor.image_id
  name  = "cadvisor"
  provider = docker.ixp_ams01
  log_driver = "json-file"
  log_opts = {
